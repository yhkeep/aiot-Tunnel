<template>
  <div class='password_manage_wrap'>
      ...
      <div class="password_manage_box">
          ...
          <div class="input_item">
              <Button type="error" id="modify_btn" @click="handleModifyFn">确认修改</Button>
          </div>
  ...
  methods:{
    ...
    handleModifyFn(){
        ...
        if(this.verifyCurrPass&&this.verifySetPass&&this.verifyCerPass){
                ...
                this.$axios.post(process.env.API_HOST+'/user/update',datas)
                .then((res)=>{
                    console.log(res)
                    if(res.data.msg==='ok'){
                        this.successFn();
...
<style scoped>
    @import "../../assets/css/password_manage.css"
</style>