...
@ServerEndpoint(value = "/map/websocket/{verification}")  //携带楼层，进行过滤
@Component
public class MapWebSocket {
	...
	在外部可以获取此连接的所有websocket对象，并能对其触发消息发送功能，定时发送核心功能的实现在与此变量 */
    private static CopyOnWriteArraySet<MapWebSocket> webSocketSet = new CopyOnWriteArraySet<MapWebSocket>();
...
//    	LogutilExcetion.logger.error(error+"/map/websocket自定义制造异常");
    }
...
//定时任务核心代码
    public static CopyOnWriteArraySet<MapWebSocket> getWebSocketSet() {
        return webSocketSet;
    }

    public static void setWebSocketSet(CopyOnWriteArraySet<MapWebSocket> webSocketSet) {
    	MapWebSocket.webSocketSet = webSocketSet;
    }
